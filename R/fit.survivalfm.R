#' Fit survivalFM and optimize regularization parameters
#'
#' This function automatically optimizes the regularization parameters
#' \code{lambda1} and \code{lambda2} for survivalFM. It partitions the input
#' dataset into a validation set and a training set, leaving the validation set
#' out of the initial training process.
#'
...
#'   case this should be set to \code{FALSE}. Default is \code{TRUE}.
...
#'
#' @return Returns an object of class \code{"fit.survivalfm"}. It is a list
...
fit.survivalfm <- function(
  ...
  if (parallel) {
    val.results <- foreach(param_idx = 1:nrow(param_combinations), .combine = 'rbind', .inorder = FALSE) %dopar% {
  ...
  return(
    structure(
      ...
      class = c("fit.survivalfm", "survivalfm")
    )
  )
